...
@torch.no_grad()
def mix_utterances(wav1: torch.Tensor, wav2: torch.Tensor, max_overlap: float = 0.5) -> torch.Tensor:
    """Mix two utterances with random overlap."""
    L1, L2 = wav1.shape[-1], wav2.shape[-1]
    mix_len = random.randint(1, max(1, int(L1 * max_overlap)))

    start1 = random.randint(0, max(1, L1 - mix_len))
    start2 = random.randint(0, max(1, L2 - mix_len)) if L2 > mix_len else 0
    actual_len = min(mix_len, L1 - start1, L2 - start2)

    if actual_len <= 0:
        return wav1

    region1 = wav1[..., start1:start1 + actual_len]
    region2 = wav2[..., start2:start2 + actual_len]
    ...
    mixed = wav1.clone()
    mixed[..., start1:start1 + actual_len] += scale * region2
    return mixed
...
class DenoiseAugmentor:
    ...
    def __init__(self, p_noise: float = 0.25, p_mix: float = 0.25,
                 snr_range: tuple = (-5, 20), buffer_size: int = 64):
    ...
    def _update_buffer(self, wav_batch: torch.Tensor):
        for i in range(wav_batch.shape[0]):
            if len(self.buffer) >= self.buffer_size:
                self.buffer.pop(0)
            self.buffer.append(wav_batch[i].detach().cpu().clone())

    @torch.no_grad()
    def __call__(self, wav_batch: torch.Tensor):
        """
        Args:
            wav_batch: [B, 1, T]
        Returns:
        ...
        for b in range(B):
            ...
            if r < self.p_mix:
                ...
                noise = random.choice(self.buffer).to(device)
                if noise.shape[-1] < T:
                ...
                augmented[b] = mix_at_snr(wav_batch[b], noise, snr)
