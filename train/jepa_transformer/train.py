#!/usr/bin/env python3
"""
JEPA training with WavLM-style Transformer architecture.
...
def train(args, temporal_shifts=None):
    """Main training loop."""
    if temporal_shifts is None:
        temporal_shifts = []
    
    os.makedirs(args.output_dir, exist_ok=True)
    
    hop = 320  # WavLM fixed stride
    ...
    while global_step < args.max_steps:
        ...
        # Online forward
        z_online, z_pred, cluster_logits = engine.module(wav_aug, mask)
...
def parse_args():
    parser = argparse.ArgumentParser(description="JEPA training with WavLM-style Transformer")
    ...
    # Architecture (WavLM-style)
    parser.add_argument('--code_dim', type=int, default=768, help='Transformer dimension')
    parser.add_argument('--conv_dim', type=int, default=512, help='CNN dimension')
    parser.add_argument('--num_heads', type=int, default=12, help='Attention heads')
    parser.add_argument('--ff_dim', type=int, default=3072, help='FFN dimension')
    parser.add_argument('--num_layers', type=int, default=12, help='Transformer layers')
...
def main():
    ...
    # Parse temporal shifts
    temporal_shifts = [int(x) for x in args.temporal_shifts.split(',') if x.strip()] if args.temporal_shifts else []
    
    if rank0():
        print("=" * 60)
        print(f"[JEPA Transformer] K={args.K}")
